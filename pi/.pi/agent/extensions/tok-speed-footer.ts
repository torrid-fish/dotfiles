...
// Bar glyphs. Both are East Asian Ambiguous, which visibleWidth() counts as
// one cell each — same as the "#"/"." they replace, so layout padding is unchanged.
const BAR_FILLED = "█";
const BAR_EMPTY = "░";
...
function renderFooter(ctx: any, width: number, theme: any): string[] {
  ...
  const modelStr = provider && !model.includes("/")
    ? (providerColor ? colorRgb(provider, providerColor) : "") + theme.fg("dim", "/") + theme.fg("accent", model)
  ...
  const elapsedSec = streamingStartTime > 0
  ...
  const bar = colorRgb(BAR_FILLED.repeat(filled), interpolateColor(pct / 100, CONTEXT_STOPS)) +
              theme.fg("dim", BAR_EMPTY.repeat(BLOCKS - filled));
